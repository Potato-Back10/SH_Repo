...
public class BattleService
{
    ...
    //전투시작
    public async Task<BattleSession> StartBattle(
        int playerId,
        int[] monsterId,
    ...
    public async Task<(bool finished, List<BattleParticipant> snap)> TickAsync(
        Guid battleId,
        int elapsedMs,
    ...
    {
        var s = await _db
            .Battles.Include(x => x.Participants)
            .FirstAsync(x => x.Id == battleId, ct);

        if (s.IsFinished)
            return (true, s.Participants.ToList());
        ...
        BattleParticipant? SelectTarget(BattleParticipant actor)
        {
            ...
            if (actor.Team == TeamSide.Players)
                return s
                    .Participants.Where(p => p.Team == TeamSide.Monsters && p.IsAlive)
                    .OrderBy(p => p.HP)
                    .FirstOrDefault();
            else
                return s
                    .Participants.Where(p => p.Team == TeamSide.Players && p.IsAlive)
                    .OrderBy(p => p.HP)
                    .FirstOrDefault();
        }
        ...
        foreach (var a in s.Participants.Where(p => p.IsAlive && p.AttackCoolTime <= 0))
        {
            ...
            var aps = Math.Max(0.1f, a.AttackSpeed);
        ...
        if (!playerAlive || !monsterAlive)
        {
            ...
            if (playerAlive && !monsterAlive)
            {
                ...
                var monsterIds = s
                    .Participants.Where(p => p.Team == TeamSide.Monsters)
                    .Select(p => p.ActorId)
                    .ToArray();
